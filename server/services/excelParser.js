...
const determineBankType = (worksheet) => {
  ...
  worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    if (rowNumber <= 10) { // Check only first few rows
      if (rowText.includes('HDFC BANK') || rowText.includes('HDFC Bank')) {
        bankType = 'HDFC';
      } else if (rowText.includes('ICICI BANK') || rowText.includes('ICICI Bank')) {
        bankType = 'ICICI';
...
const parseHDFCExcel = (worksheet) => {
  ...
  headerRowObj.eachCell({ includeEmpty: false }, (cell, colNumber) => {
    ...
    if (value.includes('date')) {
      dateCol = colNumber;
    } else if (value.includes('narration') || value.includes('description') || value.includes('particulars')) {
      descCol = colNumber;
    } else if (value.includes('amount')) {
      amountCol = colNumber;
    } else if (value.includes('type') || value.includes('cr/dr') || value.includes('debit/credit')) {
      typeCol = colNumber;
    }
  });
    ...
    if (rowNumber > headerRow) {
      ...
      if (typeCol > 0) {
        ...
        if (typeValue) {
          ...
          transactionType = (typeStr.includes('cr') || typeStr.includes('credit')) ? 'credit' : 'debit';
        }
      }
...
const parseICICIExcel = (worksheet) => {
  ...
  return parseGenericExcel(worksheet, 'ICICI');
};
...
const parseSBIExcel = (worksheet) => {
  ...
  // This is a placeholder implementation
  return parseGenericExcel(worksheet, 'SBI');
};
...
const parseGenericExcel = (worksheet, bankName = 'UNKNOWN') => {
  ...
  headerRow.eachCell({ includeEmpty: false }, (cell, colNumber) => {
    ...
    if (value.includes('date')) {
      ...
      amountCol = colNumber;
    } else if (value.includes('description') || value.includes('narration') || value.includes('particulars')) {
      descCol = colNumber;
    }
  });
    ...
    worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      if (rowNumber > 1) { // Skip header
        row.eachCell({ includeEmpty: false }, (cell, colNumber) => {
          ...
          if (typeof value === 'number' || (typeof value === 'string' && /[\d,]+\.\d{2}/.test(value))) {
            amountCol = colNumber;
