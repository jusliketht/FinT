  ...
  try {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        await sharp(tempSvgPath).png().toFile(tempImgPath);
        ...
        const {
          data: { text },
        } = await Tesseract.recognize(imgToOcr, "eng");
        ...
        try {
          ...
          console.warn(
            `Error cleaning up temp files for page ${i}:`,
            cleanupError,
          );
        }
      }
...
const detectBankFromText = (text) => {
  ...
  if (textLower.includes("hdfc bank") || textLower.includes("hdfc")) {
    ...
    textLower.includes("state bank of india") ||
...
const parseHDFCOCRText = (text) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (typeMatch) {
      transactionType =
        typeMatch[1].toLowerCase() === "cr" ? "credit" : "debit";
    }
